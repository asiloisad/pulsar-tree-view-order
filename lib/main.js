...
export default {
  ...
  install() {
    ...
    for (let root of this.treeView.roots) {
      if (!sortEntriesOriginal) {
        sortEntriesOriginal = root.directory.constructor.prototype.sortEntries
      }
  ...
  deactivate() {
    ...
    if (this.treeView && sortEntriesOriginal) {
      for (let root of this.treeView.roots) {
        root.directory.constructor.prototype.sortEntries = sortEntriesOriginal
      }
    }
    this.treeView.updateRoots()
    this.treeView = null
  ...
  setSortMethod(sortMethod) {
    if (sortMethod===1) {
      sorterFn = require('natural-compare-lite')
    } else if (sortMethod===2) {
      sorterFn = require('natsort').default()
    }
  },
}

let sortEntriesOriginal = null ; let sortBefore = null
