...
/**
 * Tree View Order Package
 * Provides customizable natural sort ordering for the tree-view.
 * Supports multiple sorting algorithms and base name sorting.
 */
...
module.exports = {
  /**
   * Activates the package and installs the custom sort function.
   */
  activate() {
    ...
    this.disposables.add(
      atom.config.observe("tree-view-order.sortMethod", (value) => {
        sortMethod = value;
        if (sortMethod === 0) {
          sortCompare = Intl.Collator(undefined, {
            numeric: true,
            sensitivity: "base",
          }).compare;
  ...
  /**
   * Installs the custom sort function into the tree-view.
   */
  install() {
    if (this.treeView) {
      return;
  ...
  /**
   * Deactivates the package and restores the original sort function.
   */
  deactivate() {
